...
int main (int argc, char *argv[])
{
  ...
  // Create folder to put debugging files in
  int err = mkdir("debugfiles",0755);
  if (err<0 && errno==EEXIST){
	  // Debug folder already exists --> clean debug files
	  remove("debugfiles/subBands.txt");
  }
  ...
  for (bufPos=0; bufPos<input.samplesAvailable; bufPos+=(BUFFERSIZE/2)){
	  ...
	  writeBufferToTextFile(buffer,"debugfiles/subBands.txt");
